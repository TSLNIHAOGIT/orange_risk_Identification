...
class rfe_feature_select(object):

    def __init__(self,X_loc_train, y_loc_train,X_loc_test):
        self.X_loc_train=X_loc_train
        self.y_loc_train=y_loc_train
        self.X_loc_test=X_loc_test

    def feature_select(self,X_loc_train, y_loc_train,X_loc_test):
        feature_select = RFE(estimator=RandomForestClassifier(
                           n_estimators=500,
                           criterion="gini",
                           max_depth=8,
                           min_samples_split=2,
        ...
        return X_loc_train,X_loc_test
    def main(self):
        X_loc_train, X_loc_test=self.feature_select(self.X_loc_train, self.y_loc_train, self.X_loc_test)
        return X_loc_train,X_loc_test
...
class IG():
    def __init__(self,X,y,X_test,percentile=0.95):
        X = np.array(X)
    ...
    def main(self):
        info_gain=self.getIG()
        select_feature_name=self.select_feature(self.X,info_gain,self.percentile)
        X_loc_train = self.X[select_feature_name].values
        X_loc_test = self.test[select_feature_name].values
        return X_loc_train, X_loc_test
...
class FeatureSelector(object):
    ...
    def __init__(self, data, labels=None):
        ...
        self.data = data
        self.labels = labels
        self.scale_pos_weight=len(labels[labels==0])/len(labels[labels==1])
        print('self.scale_pos_weight',self.scale_pos_weight)
        if labels is None:
    ...
    def identify_zero_importance(self, task, eval_metric=None,
        ...
        for _ in range(n_iterations):
            ...
            if task == 'classification':
                model = lgb.LGBMClassifier(n_estimators=1000, learning_rate=0.05, verbose=-1,scale_pos_weight=self.scale_pos_weight)
    ...
    def remove(self, methods, keep_one_hot=True):
        ...
        if methods == 'all':
            ...
            # Need to use one-hot encoded data as well
            if False:  # 'zero_importance' in methods or 'low_importance' in methods or self.one_hot_correlated:
                data = self.data_all
                pass
        ...
        return data, self.removed_features
...
if __name__=='__main__':
    ...
    fs.identify_all(selection_params={'missing_threshold': 0.7,
                                      ...
                                      'task': 'classification',
                                      'eval_metric': tpr_weight_funtion_lc,#注意lgbclassifier的参数
                                      'cumulative_importance': 0.999
                                      })

    left_feature, removed_feature = fs.remove(
        methods=['missing', 'single_unique', 'collinear', 'zero_importance', 'low_importance'], keep_one_hot=True)

    X_loc_train = train.drop(labels=removed_feature, axis=1)
    X_loc_test = test.drop(labels=removed_feature, axis=1)

    print('特征选择完成，开始保存数据')
    X_loc_train.to_csv(path0+'train_select.csv',index=False)
    X_loc_test.to_csv(path0+'test_select.csv',index=False)
    print('数据已经保存')
